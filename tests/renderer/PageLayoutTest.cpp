...
void run() {
  ...
  request.profile.mode = LayoutMode::Paged;
  std::string source = "# Page test\n\n";
  for (int i = 0; i < 100; ++i)
    source += "Paragraph " + std::to_string(i) + " with æøå and **bold** content.\n\n";
  source += "```\n";
  for (int i = 0; i < 250; ++i)
    source += "é";
  source += "\n```\n";
  ...
  for (const auto& run : frame->runs) {
    ...
    CHECK(y + run.bounds.height <=
          request.profile.paper.height - request.profile.paper.margin + .01);
    CHECK(run.bounds.x + run.bounds.width <=
          request.profile.paper.width - request.profile.paper.margin + .01);
    if (run.font.mono && !run.text.empty())
      CHECK((static_cast<unsigned char>(run.text.front()) & 0xc0) != 0x80);
  }
  auto wider = request;
  wider.width = 1200;
  ...
  for (std::size_t i = 0; i < frame->runs.size(); ++i)
    CHECK(same->runs[i].bounds.y == frame->runs[i].bounds.y);
  auto empty = renderer.layout(*parser.parse({{2, 0}, "", {}, false}), request, metrics);
  CHECK(empty->pages.slices.size() == 1);
  FlowLayout lines;
  lines.lines = {{0, 30, 0}, {30, 20, 2}, {50, 20, 0}, {70, 20, 0}};
  ...
  try {
    renderer.layout(*model, request, metrics);
  } catch (const Error&) {
    cancelled = true;
  }
  CHECK(cancelled);
  ...
  bool invalid = false;
  try {
    PageBreaker::breakLines(lines, paper);
  } catch (const Error&) {
    invalid = true;
  }
  CHECK(invalid);

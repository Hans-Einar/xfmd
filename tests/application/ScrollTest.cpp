...
void run() {
  ...
  Metrics metrics;
  std::string source = "# Heading\n\nA paragraph with repeated repeated words and many words that "
                       "wrap.\n\n```\nfirst\nrepeat\nrepeat\nlast\n```\n";
  ...
  int previewY = -1, moves = 0;
  std::size_t editorByte = 0;
  coordinator.setPreview = [&](int y) {
    previewY = y;
    ++moves;
    coordinator.onViewportChanged(ViewOrigin::Preview, y, {1, 2});
  };
  coordinator.setEditor = [&](SourceAnchor anchor) {
    editorByte = anchor.byte;
    ++moves;
    coordinator.onViewportChanged(ViewOrigin::Editor, anchor.byte, {1, 2});
  };
  coordinator.setSplit(true);
  coordinator.invalidate({1, 2});
  ...
  coordinator.onViewportChanged(ViewOrigin::Editor, second, {1, 2}, 1);
  CHECK(moves == 2);
  coordinator.onViewportChanged(ViewOrigin::Editor, second, {1, 2}, 3, true);
  CHECK(moves == 2);
  coordinator.invalidate({1, 3});
  coordinator.onViewportChanged(ViewOrigin::Editor, second, {1, 2}, 4);
  CHECK(moves == 2);
  coordinator.setFrame(frame);
  CHECK(moves == 2);

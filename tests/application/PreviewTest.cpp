...
struct TestInterpreter : IInterpreter {
  ...
  ParseResult parse(const SourceSnapshot& source, const ParseOptions&) override {
    ...
    SemanticBlock block;
    block.source = {0, source.text.size()};
...
void run() {
  ...
  TestInterpreter parser;
  MarkdownRenderer renderer;
  Metrics metrics;
  FakeScheduler clock;
  LayoutResult frame;
  int invalidations = 0, failures = 0;
  {
    ...
    session.applyEdit({0, 0, "one"});
    preview.schedule();
    clock.advance(100);
    session.applyEdit({0, 3, "two"});
    preview.schedule();
    clock.advance(150);
    session.applyEdit({0, 3, "three"});
    preview.schedule();
    clock.advance(299);
    CHECK(parser.calls == 0 && !frame);
    clock.advance(1);
    waitUntil([&] {
      preview.poll();
      return !preview.busy();
    ...
    preview.relayout(320);
    CHECK(parser.calls == 1 && frame->width == 320);
    parser.intercept = [](const SourceSnapshot&) {
      throw Error(ErrorCode::Parse, "injected parse failure");
    };
    session.applyEdit({0, 5, "bad"});
    preview.refresh();
    waitUntil([&] {
      preview.poll();
      return !preview.busy();
  ...
  delayed.intercept = [&](const SourceSnapshot& source) {
  ...
  worker.submit({{5, 1}, "first", {}, false});
  entered.get_future().wait();
  worker.submit({{5, 2}, "superseded", {}, false});
  worker.submit({{6, 0}, "newest", {}, false});
  release.set_value();
  std::optional<ParseCompletion> result;
  waitUntil([&] {
    const bool pending = worker.busy();
    result = worker.take();
    if (result)
      CHECK(pending);

...
void run() {
  char pattern[] = "/tmp/xfmd-nav-XXXXXX"; std::filesystem::path dir = mkdtemp(pattern);
  struct Cleanup { std::filesystem::path path; ~Cleanup() { std::filesystem::remove_all(path); } } cleanup{dir};
  for (const char* name : {"a.md", "b space.md", "c.md", "d.md"}) std::ofstream(dir / name) << name;
  DocumentSession session; LocalFileStore files; DocumentCoordinator docs(session, files); ScrollCoordinator scroll;
  NavigationCoordinator nav(docs, session, scroll);
  int errors = 0, commits = 0;
  docs.error = [&](const std::string&) { ++errors; }; nav.error = docs.error;
  docs.opened = [&] { ++commits; nav.commitVisit(); };
  ...
  for (const char* target : {"https://example.org/a.md", "javascript:alert(1)", "data:text/plain,a", "//host/a.md", "a.md#title", "%00.md", "a%xx.md", "javascript%3Aevil.md"}) {
    CHECK(!nav.followLink(target)); CHECK(commits == before);
  }
  CHECK(nav.goBack() && session.view().path == (dir / "b space.md").string());
  CHECK(scroll.captureAnchor().byte == nav.history.at(1).anchor.byte);
  auto count = nav.history.size(); CHECK(nav.openTarget(session.view().path)); CHECK(nav.history.size() == count);
  CHECK(errors >= 9);
  HistoryStore many;
  for (int i=0;i<105;++i) many.commit(std::to_string(i), {});
  CHECK(many.size() == 100 && many.position() == 99 && many.at(0).path == "5");
  CHECK(LinkResolver::resolve((dir / "a.md").string(), "../file.md") == (dir.parent_path() / "file.md").string());
}
TEST_MAIN(run)

...
int main(int argc,char** argv) {
  try {
    ...
    xfmd::SharedTextMetrics metrics;xfmd::CmarkInterpreter parser;xfmd::MarkdownRenderer renderer;
    auto model=parser.parse({{1,1},"# XFMD\n\nUnicode æøå office é 日本語\n\n**Bold** and `code`.\n",{},false});
    ...
    auto pdf=(directory/"typography.pdf").string();auto png=(directory/"typography.png").string();
    ...
    for(auto* target:{surface,image}) {
      auto* cr=cairo_create(target);cairo_set_source_rgb(cr,1,1,1);cairo_paint(cr);
      xfmd::DisplayListPainter painter(metrics.catalog);painter.paint(*frame,cr,{0,0,595.276,841.89});
      CHECK(cairo_status(cr)==CAIRO_STATUS_SUCCESS);cairo_destroy(cr);
    }
    CHECK(cairo_surface_write_to_png(image,png.c_str())==CAIRO_STATUS_SUCCESS);
    cairo_surface_finish(surface);CHECK(cairo_surface_status(surface)==CAIRO_STATUS_SUCCESS);
    cairo_surface_destroy(image);cairo_surface_destroy(surface);
    // A fresh, thread-local catalog can replay the same immutable frame.
    xfmd::FontCatalog other;
    CHECK(other.id()==metrics.catalog.id());
    for(const auto& run:frame->runs)for(const auto& segment:run.shaped->segments)CHECK(other.font(segment.fontFace));

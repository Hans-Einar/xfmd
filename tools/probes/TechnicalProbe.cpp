...
int main(int argc, char** argv) {
  ...
  app.init(argc, argv);
  auto* window = new FX::FXMainWindow(&app, "xfmd P0: native fonts and source proof", nullptr,
                                      nullptr, FX::DECOR_ALL, 0, 0, 800, 450);
  auto* column = new FX::FXVerticalFrame(window, FX::LAYOUT_FILL_X | FX::LAYOUT_FILL_Y);
  std::vector<FX::FXFont*> fonts;
  for (const auto& item : std::vector<std::pair<std::string, int>>{{"DejaVu Sans", 24},
                                                                   {"DejaVu Sans", 18},
                                                                   {"DejaVu Serif", 12},
                                                                   {"DejaVu Sans Mono", 11}}) {
    ...
    auto* label =
        new FX::FXLabel(column, "Heading / brødtekst / code — æøå", nullptr, FX::JUSTIFY_LEFT);
    label->setFont(font);
  }
  ...
  for (auto* font : fonts) {
    ...
    std::cout << "font height=" << font->getFontHeight()
              << " width=" << font->getTextWidth("abc", 3) << '\n';
  }
  const std::string md =
      "# æøå\n\nrepeat &amp; **repeat**\n\n> - item\n>   - nested\n\n```\nrepeat\n```\n";
  ...
  while (cmark_iter_next(it) != CMARK_EVENT_DONE) {
    if (cmark_iter_get_event_type(it) != CMARK_EVENT_ENTER)
      continue;
    auto* node = cmark_iter_get_node(it);
    std::cout << cmark_node_get_type_string(node) << ' ' << cmark_node_get_start_line(node) << ':'
              << cmark_node_get_start_column(node) << '-' << cmark_node_get_end_line(node) << ':'
              << cmark_node_get_end_column(node) << '\n';
  }
  cmark_iter_free(it);
  cmark_node_free(doc);
  std::string large;
  while (large.size() < 1024 * 1024)
    large += md;
  ...
  std::cout << "cmark=" << cmark_version_string() << " parse+free p95_ms=" << samples[28]
            << " bytes=" << large.size() << '\n';

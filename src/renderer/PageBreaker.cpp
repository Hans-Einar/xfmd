...
#include <algorithm>
namespace xfmd {
std::vector<PageSlice> PageBreaker::breakLines(const FlowLayout& flow,const PaperSpec& paper,const std::function<bool()>& cancelled) {
  paper.validate();std::vector<PageSlice> pages;
  ...
  for(std::size_t i=0;i<flow.lines.size();++i) {
    ...
    if(!std::isfinite(line.top)||!std::isfinite(line.height)||line.height<=0 || line.height>available)
    ...
    if(!needsPage && line.keepFollowing && groupBottom-line.top<=available && groupBottom-pages.back().flowTop>available)
      needsPage=true;
    if(needsPage) {
      if(pages.size()>=2000)throw Error(ErrorCode::TooLarge,"Document exceeds the 2000-page limit.");
      pages.push_back({line.top,line.top+line.height,pages.size()});
    } else pages.back().flowBottom=line.top+line.height;
  }
  if(pages.empty())pages.push_back({0,0,0});
  return pages;
}
} // namespace xfmd

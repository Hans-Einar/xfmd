    ...
    for (std::size_t begin = 0; begin < run.text.size();) {
      ...
      if (!code && measured > width) {
        ...
        if (auto shaped = metrics.shape(word, font)) {
          for (const auto& part : shaped->segments)
        ...
        while (begin < end) {
          ...
          if (lower == boundaries.end())
            throw Error(ErrorCode::Layout, "Invalid text cluster mapping.");
          // Binary search avoids quadratic shaping of long unbroken words.
          auto best = lower;
          auto hi = boundaries.end();
          while (lower < hi) {
            auto mid = lower + (hi - lower) / 2;
            if (metrics
                    .measure(std::string_view(run.text).substr(begin, wordStart + *mid - begin),
                             font)
                    .width <= width) {
              best = mid;
              lower = mid + 1;

...
HitResult HitTester::hitTest(const RenderFrame& frame, Point point) {
  auto first = std::lower_bound(frame.runs.begin(), frame.runs.end(), point.y - frame.maxRunHeight,
                                [](const DrawRun& run, double y) { return run.bounds.y < y; });
  for (; first != frame.runs.end() && first->bounds.y <= point.y; ++first)
    if (!first->link.empty() && first->bounds.contains(point))
      return {first->link, first->source};
  return {};
}
} // namespace xfmd

...
bool identity(const std::string& s) {
  return !s.empty() && s.size() <= 64 &&
         s.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_") ==
             std::string::npos;
}
sockaddr_un address(const std::string& path) {
  ...
  if (path.size() >= sizeof(a.sun_path))
    throw std::runtime_error("IPC path too long");
...
WindowEndpoint::WindowEndpoint(const std::string& id) : window(id) {
  ...
  if (listener < 0)
    throw std::runtime_error("Cannot create window endpoint");
  if (bind(listener, reinterpret_cast<sockaddr*>(&a), sizeof(a)) || listen(listener, 16)) {
    close(listener);
    listener = -1;
    throw std::runtime_error("Window ID is already registered or unavailable");
  }
}
...
std::string WindowEndpoint::handle(const std::string& message) {
  ...
  try {
    if (f[4].empty() || f[4].find_first_not_of("0123456789") != std::string::npos)
      throw std::runtime_error("sequence");
    seq = std::stoull(f[4]);
  } catch (...) {
    return "ERROR\tinvalid sequence\n";
  }
  auto key = f[3] + ":" + f[5];
  if (sequences.size() >= 256 && !sequences.count(key))
    return "ERROR\tclient limit\n";
  if (seq <= sequences[key])
...
void WindowEndpoint::poll() {
  for (int i = 0; i < 16; i++) {
    ...
    if (clients.size() >= 16 || getsockopt(fd, SOL_SOCKET, SO_PEERCRED, &cred, &length) ||
        cred.uid != getuid()) {
      close(fd);
      continue;
    }
  ...
  for (auto it = clients.begin(); it != clients.end();) {
    char buffer[32769];
    auto n = recv(it->fd, buffer, sizeof(buffer), MSG_DONTWAIT | MSG_TRUNC);
    ...
    if (n > 0 && n <= 32768) {
      try {
        response = handle(std::string(buffer, n));
      } catch (const std::exception& e) {
        response = "ERROR\tdocument failure\n";
      }
    }
    send(it->fd, response.data(), response.size(), MSG_NOSIGNAL);
    close(it->fd);
    it = clients.erase(it);
  }
}
std::string WindowEndpoint::request(const std::string& id, const std::string& message) {
  if (!identity(id) || message.size() > 32768)
    throw std::runtime_error("Invalid window request");
  auto a = address(runtimeDirectory() + "/" + id + ".sock");
  int fd = socket(AF_UNIX, SOCK_SEQPACKET | SOCK_CLOEXEC, 0);
  if (fd < 0)
    throw std::runtime_error("IPC socket failed");
  struct Guard {
    int fd;
    ~Guard() { close(fd); }
  } guard{fd};
  if (connect(fd, reinterpret_cast<sockaddr*>(&a), sizeof(a)))
    throw std::runtime_error("Target window is closed or unavailable");
  if (send(fd, message.data(), message.size(), MSG_NOSIGNAL) !=
      static_cast<ssize_t>(message.size()))
    throw std::runtime_error("IPC send failed");
  pollfd p{fd, POLLIN, 0};
  if (::poll(&p, 1, 5000) <= 0)
    throw std::runtime_error("Window acknowledgement timed out");
  char b[32769];
  auto n = recv(fd, b, sizeof(b), MSG_TRUNC);
  if (n <= 0 || n > 32768)
    throw std::runtime_error("Invalid window response");

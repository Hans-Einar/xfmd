...
XfmdWindow::XfmdWindow(FXApp* app, CommandRouter& router)
    : FXMainWindow(app, "xfmd", nullptr, nullptr, DECOR_ALL, 0, 0, 1100, 760), commands(&router) { buildUi(); }
XfmdWindow::~XfmdWindow() { delete fileMenu; delete editMenu; delete viewMenu; delete goMenu; }
void XfmdWindow::buildUi() {
  auto* bar = new FXMenuBar(this, LAYOUT_SIDE_TOP | LAYOUT_FILL_X);
  auto add = [&](FXMenuPane* menu, const char* name, CommandRouter::Command command) { new FXMenuCommand(menu, name, nullptr, commands, command); };
  ...
  auto* toolbar = new FXHorizontalFrame(this, LAYOUT_SIDE_TOP | LAYOUT_FILL_X | PACK_UNIFORM_HEIGHT);
  for (const auto& item : {std::pair<const char*, CommandRouter::Command>{"Open", CommandRouter::Open}, {"Save", CommandRouter::Save}, {"Back", CommandRouter::Back}, {"Forward", CommandRouter::Forward}, {"Preview", CommandRouter::Preview}, {"Split", CommandRouter::Split}, {"Editor", CommandRouter::Editor}})
    new FXButton(toolbar, item.first, nullptr, commands, item.second, BUTTON_NORMAL);
  status = new FXLabel(this, "Open a local Markdown or text file.", nullptr, LAYOUT_SIDE_BOTTOM | LAYOUT_FILL_X | JUSTIFY_LEFT);
  auto* workspace = new FXSplitter(this, SPLITTER_HORIZONTAL | SPLITTER_TRACKING | LAYOUT_FILL_X | LAYOUT_FILL_Y);
  sidebar = new SidebarWidget(workspace);
  split = new FXSplitter(workspace, SPLITTER_HORIZONTAL | SPLITTER_TRACKING | LAYOUT_FILL_X | LAYOUT_FILL_Y);

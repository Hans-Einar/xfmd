...
SidebarWidget::SidebarWidget(FXComposite* parent)
    : FXDirList(parent, this, 1, DIRLIST_SHOWFILES | TREELIST_SHOWS_LINES | TREELIST_SHOWS_BOXES | LAYOUT_FILL_Y, 0, 0, 220, 0) {
  setPattern("*.md,*.txt");
...
long SidebarWidget::onOpen(FXObject*, FXSelector, void* data) {
  ...
  if (InputPolicy::supportedPath(path) && open) open(path);
  return 1;
}
}

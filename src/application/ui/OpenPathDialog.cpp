...
OpenPathDialog::OpenPathDialog(FXWindow* owner, const std::string& directory)
    : FXFileDialog(owner, "Open file or folder") {
  ...
  resize(600, 400);
  new FXButton(row, "Open current &folder", nullptr, this, ID_CURRENT_FOLDER,
...
long OpenPathDialog::onAccept(FXObject* sender, FXSelector selector, void* data) {
  ...
  if (!paths || paths[0].empty() || !paths[1].empty()) {
    feedback->setText("Select one file or folder, or use Open current folder.");
    return 1;
  }
  selected = paths[0];

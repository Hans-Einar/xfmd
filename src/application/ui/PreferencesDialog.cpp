...
using namespace FX;
namespace xfmd {
FXDEFMAP(PreferencesDialog) preferencesMap[]={
...
PreferencesDialog::PreferencesDialog(FXWindow* owner,PreferencesService& preferences)
  ...
  auto number=[&](const char* label,double value,double min,double max,double step) {
    ...
    input->setRange(min,max);input->setIncrement(step);input->setValue(value);return input;
  };
  ...
  sample=new EditorWidget(content);sample->setEditable(false);
  sample->setHeight(150);sample->setLayoutHints(LAYOUT_FILL_X|LAYOUT_FILL_Y);
  FXString text;
  for(int i=1;i<=100;++i) { FXString row;row.format("Scroll sample %03d — slow and fast movement\n",i);text+=row; }
  sample->setText(text);
  ...
  new FXButton(buttons,"&OK",nullptr,this,ID_ACCEPT,BUTTON_INITIAL|BUTTON_DEFAULT|BUTTON_NORMAL|LAYOUT_RIGHT);
  onChanged(nullptr,0,nullptr);
}
long PreferencesDialog::onChanged(FXObject*,FXSelector,void*) {
  draft.scroll={speed->getValue(),bool(acceleration->getCheck()),strength->getValue(),maximum->getValue()};
...
long PreferencesDialog::onAccept(FXObject*,FXSelector,void*) {
  onChanged(nullptr,0,nullptr);
  std::string message;
  if(!service->commit(draft,message)) {error->setText(message.c_str());return 1;}

...
std::size_t TextProjection::sourceOffset(std::size_t projected) const {
  return offsets[std::min(projected, text.size())];
}
std::size_t TextProjection::displayOffset(std::size_t source) const {
  return std::min(
      text.size(),
      std::size_t(std::lower_bound(offsets.begin(), offsets.end(), source) - offsets.begin()));
}
Edit TextProjection::difference(const std::string& edited) const {
  std::size_t begin = 0, oldEnd = text.size(), newEnd = edited.size();
  while (begin < oldEnd && begin < newEnd && text[begin] == edited[begin])
    ++begin;
  // A byte diff may start inside a multi-byte character; expand to a code point boundary.
  while (begin && begin < text.size() && (static_cast<unsigned char>(text[begin]) & 0xc0) == 0x80)
    --begin;
  while (oldEnd > begin && newEnd > begin && text[oldEnd - 1] == edited[newEnd - 1]) {
    --oldEnd;
    --newEnd;
  }
  while (oldEnd < text.size() && (static_cast<unsigned char>(text[oldEnd]) & 0xc0) == 0x80) {
    ++oldEnd;
    ++newEnd;
  }

...
void EditController::apply(const Edit& edit, bool record) {
  ...
  if (previous == session.view().token)
    return;
  if (record) {
    ...
    while (historyBytes > 32 * 1024 * 1024 && undoStack.size() > 1) {
      historyBytes -= undoStack.front().forward.replacement.size() +
                      undoStack.front().backward.replacement.size();
...
void EditController::applyProjectedText(const std::string& text) {
  applyEdit(TextProjection(session.view().text).difference(text));
}

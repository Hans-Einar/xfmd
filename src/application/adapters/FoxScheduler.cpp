...
using namespace FX;
namespace xfmd {
FXDEFMAP(FoxScheduler)
schedulerMap[] = {FXMAPFUNCS(SEL_TIMEOUT, 1, 65535, FoxScheduler::onTimeout)};
...
void FoxScheduler::restart(unsigned key, unsigned milliseconds, std::function<void()> callback) {
  if (!key || key > 65535)
    throw std::invalid_argument("Timer key out of range");
  cancel(key);
  callbacks[key] = std::move(callback);
  app->addTimeout(this, key, milliseconds);
}
void FoxScheduler::cancel(unsigned key) {
  if (app)
    app->removeTimeout(this, key);
  callbacks.erase(key);
}
void FoxScheduler::cancelAll() {
  while (!callbacks.empty())
    cancel(callbacks.begin()->first);
}

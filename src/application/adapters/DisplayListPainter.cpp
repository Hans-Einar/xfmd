...
void rectangle(cairo_t* cr,Rect r) {cairo_rectangle(cr,r.x,r.y,r.width,r.height);cairo_fill(cr);}
bool visible(Rect r,Rect clip) {return r.y+r.height>=clip.y && r.y<=clip.y+clip.height && r.x+r.width>=clip.x && r.x<=clip.x+clip.width;}
}
void DisplayListPainter::text(cairo_t* cr,const DrawRun& draw) {
  ...
  for(const auto& segment:part.segments) {
    ...
    for(std::size_t i=0;i<segment.glyphs.size();++i) {
      const auto& value=segment.glyphs[i];auto& g=glyphs->glyphs[i];
      g.glyph=value.index;g.geometry.width=std::lround(value.advance*PANGO_SCALE);
      g.geometry.x_offset=std::lround(value.xOffset*PANGO_SCALE);g.geometry.y_offset=std::lround(value.yOffset*PANGO_SCALE);
      g.attr.is_cluster_start=i==0 || value.cluster!=segment.glyphs[i-1].cluster;
      glyphs->log_clusters[i]=value.cluster;
    }
    PangoGlyphItem run{};run.item=&item;run.glyphs=glyphs;
...
void DisplayListPainter::paint(const RenderFrame& frame,cairo_t* cr,Rect clip,bool active) {
  ...
  for(const auto& decoration:frame.decorations)if(visible(decoration.bounds,clip)) {
    color(cr,decoration.color);rectangle(cr,decoration.bounds);
  }
  ...
  for(;first!=frame.runs.end() && first->bounds.y<clip.y+clip.height;++first) {
    const auto& run=*first;if(!visible(run.bounds,clip))continue;
    if(run.codeBackground){color(cr,0xeff1f5);rectangle(cr,run.bounds);}
    ...
    if(run.icon==InlineIcon::Globe) {
      ...
      cairo_set_line_width(cr,.8);cairo_arc(cr,x+size/2,y+size/2,size/2,0,6.283185307);cairo_stroke(cr);
      cairo_move_to(cr,x,y+size/2);cairo_line_to(cr,x+size,y+size/2);cairo_stroke(cr);
    } else text(cr,run);
    if(!run.link.empty()) {
      cairo_set_line_width(cr,.6);cairo_move_to(cr,run.bounds.x,run.bounds.y+run.ascent+2);
      cairo_line_to(cr,run.bounds.x+run.bounds.width,run.bounds.y+run.ascent+2);cairo_stroke(cr);
    }
  }
}
} // namespace xfmd

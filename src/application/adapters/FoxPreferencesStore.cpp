...
PreferencesSnapshot FoxPreferencesStore::load() {
  ...
  auto read=[&](const char* section,const char* key,double fallback,double min,double max) {
    ...
    try {
      std::size_t count=0;std::string text(value);double result=std::stod(text,&count);
      if(count==text.size() && std::isfinite(result) && result>=min && result<=max)return result;
    } catch(...) {}
    return fallback;
  };
  const auto version=read("Preferences","version",registry.existingEntry("Preferences","version") ? -1 : 1,-1,1000000);
  readOnly=version!=1;
...
bool FoxPreferencesStore::save(const PreferencesSnapshot& s,std::string& error) {
  if(readOnly) {error="Preferences were written by a newer version; keeping them unchanged.";return false;}
  if(!PreferencesService::validate(s,error))return false;

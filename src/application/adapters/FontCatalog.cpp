...
std::string fileIdentity(FcPattern* pattern) {
  ...
  if (!file)
    return "unknown";
  struct stat st{};
  if (stat(reinterpret_cast<const char*>(file), &st) != 0)
    throw Error(ErrorCode::Layout, "Font file is unavailable.");
...
FontCatalog::FontCatalog() {
  ...
  pango_cairo_context_set_font_options(contextValue, opts);
  ...
  try {
    ...
    if (set)
      for (int i = 0; i < set->nfont; ++i)
        keys.push_back(fileIdentity(set->fonts[i]));
...
std::string FontCatalog::remember(PangoFont* value) {
  auto name = key(value);
  if (!fonts.count(name))
    fonts.emplace(name, PANGO_FONT(g_object_ref(value)));
  return name;
}
PangoFont* FontCatalog::font(const std::string& name) {
  ...
  std::unique_ptr<PangoFont, decltype(&g_object_unref)> owner(
      pango_font_map_load_font(map, contextValue, desc), g_object_unref);
  auto* value = owner.get();
  pango_font_description_free(desc);
  if (!value)
    throw Error(ErrorCode::Layout, "Unable to load document font.");
  if (key(value) != name) {
    throw Error(ErrorCode::Layout, "Document fonts changed. Refresh the preview before exporting.");
  }
  fonts.emplace(name, value);
  owner.release();
  return value;
}
} // namespace xfmd

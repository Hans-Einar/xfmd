...
void InputPolicy::validate(std::string_view text, const std::string& path) {
  ...
  if (text.size() > maxDocumentBytes) throw Error(ErrorCode::TooLarge, "File exceeds the 8 MiB limit.");
  for (std::size_t i = 0; i < text.size();) {
    auto c = static_cast<unsigned char>(text[i++]);
    if (c == 0) throw Error(ErrorCode::InvalidInput, "NUL bytes cannot be edited.");
    if (c < 0x80) continue;
    ...
    else throw Error(ErrorCode::InvalidInput, "Input is not valid UTF-8.");
    if (i + count > text.size()) throw Error(ErrorCode::InvalidInput, "Incomplete UTF-8 sequence.");
    while (count--) {
      auto next = static_cast<unsigned char>(text[i++]);
      if ((next & 0xc0) != 0x80) throw Error(ErrorCode::InvalidInput, "Input is not valid UTF-8.");

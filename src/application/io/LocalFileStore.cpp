...
FileIdentity identity(const struct stat& st, const std::string& bytes) {
  ...
  return {std::uint64_t(st.st_dev),
          std::uint64_t(st.st_ino),
          hash,
          st.st_mtim.tv_sec * 1000000000LL + st.st_mtim.tv_nsec,
          st.st_ctim.tv_sec * 1000000000LL + st.st_ctim.tv_nsec,
          bytes.size()};
}
std::string resolved(const std::string& name) {
  ...
  if (error)
    throw Error(ErrorCode::Io, "Cannot resolve file: " + error.message());
...
void copyAttributes(int from, int to) {
  ...
  if (size < 0 && (errno == ENOTSUP || errno == EOPNOTSUPP))
    return;
  if (size < 0)
    io("Read file attributes");
  std::vector<char> names(static_cast<std::size_t>(size));
  if (size && flistxattr(from, names.data(), names.size()) != size)
    io("Read attribute names");
  for (std::size_t i = 0; i < names.size();) {
    ...
    if (fgetxattr(from, name, value.data(), value.size()) != length)
      io("Read attribute value");
    if (fsetxattr(to, name, value.data(), value.size(), 0) < 0)
      io("Preserve file attributes");
    i += std::strlen(name) + 1;
  }
}
} // namespace
...
LoadedDocument LocalFileStore::read(const std::string& name) const {
  ...
  if (fstat(file.fd, &before) < 0)
    io("Inspect file");
  if (!S_ISREG(before.st_mode))
    throw Error(ErrorCode::Unsupported, "Only regular files can be opened.");
  if (before.st_size > static_cast<off_t>(maxDocumentBytes))
    throw Error(ErrorCode::TooLarge, "File exceeds 8 MiB.");
  std::string bytes;
  char buffer[16384];
  for (;;) {
...
SavedDocument LocalFileStore::writeAtomic(const SourceSnapshot& source, const std::string& name,
  ...
  if (!exists && errno != ENOENT)
    io("Inspect save target");
  if (exists && !expected)
    throw Error(ErrorCode::Conflict, "Target already exists; confirm overwrite first.");
  ...
  if (exists && original.st_nlink > 1)
    throw Error(ErrorCode::Unsupported, "File has hard links. Use Save As another name.");
  Descriptor old(exists ? ::open(path.c_str(), O_RDONLY | O_CLOEXEC | O_NOFOLLOW) : -1);
  if (exists && old.fd < 0)
    io("Open metadata source");
  ...
  if (exists) {
    if (fchown(temp.fd, original.st_uid, original.st_gid) < 0)
      io("Preserve file owner");
    if (fchmod(temp.fd, original.st_mode & 0777) < 0)
      io("Preserve file permissions");
  ...
  if (expected && !(read(path).identity == *expected))
    throw Error(ErrorCode::Conflict, "File changed before replacement.");
  if (!expected) {
    // link publishes a new file without overwriting a target created by another writer.
    if (::link(tempName.data(), path.c_str()) < 0)
      io("Publish new file");
    ::unlink(tempName.data());
  } else if (::rename(tempName.data(), path.c_str()) < 0)
    io("Replace file");
  struct stat saved{};
  if (::fstat(temp.fd, &saved) < 0)
    io("Inspect saved file");
  Descriptor directory(
      ::open(fs::path(path).parent_path().c_str(), O_RDONLY | O_DIRECTORY | O_CLOEXEC));

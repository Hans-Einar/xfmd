...
int main(int argc, char** argv) {
  try {
    ...
    for (int i = 1; i < argc; i++) {
      ...
      if (a == "--help") {
        std::cout << "Usage: xfmd [file|directory] [--navigator file.md --sdl-tool program "
                     "--sdl-source model --project ID --renderer program --window-id ID]\n       "
                     "xfmd --window ID --pane main|navigation [--client ID --request N] file.md\n  "
                     "     xfmd --window ID --info\n";
        return 0;
      }
      if (a == "--navigator")
        config.navigator = value();
      else if (a == "--sdl-tool")
        config.tool = value();
      else if (a == "--sdl-source")
        config.source = value();
      else if (a == "--project")
        ...
        info = true;
      else if (a.rfind("--", 0) == 0 || !path.empty())
        throw std::runtime_error("Unknown or duplicate argument");
      else
        path = a;
    }
    if (!target.empty()) {
    ...
    if (config.window.empty())
      config.window = std::to_string(getpid()) + "-" +
                      std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
    xfmd::Application application;
    int foxargc = 1;
    char* foxargv[] = {argv[0], nullptr};
    application.initialize(foxargc, foxargv);
    if (!path.empty() && !application.startPath(path)) {
      std::cerr << application.window->status->getText().text() << '\n';
      return 1;
    }

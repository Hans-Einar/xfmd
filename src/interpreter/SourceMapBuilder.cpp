...
std::size_t SourceMapBuilder::offset(int line, int column, bool end) const {
  if (line < 1 || std::size_t(line) > lines.size())
    return source.size();
...
SourceRange SourceMapBuilder::record(cmark_node* node) const {
  ...
  // intra-run positions.
  auto quality =
      std::string_view(source).substr(begin, end - begin).find('\t') != std::string_view::npos
          ? MappingQuality::Approximate
          : MappingQuality::Exact;
  return {begin, std::max(begin, end), quality};
}
std::vector<InlineRun> SourceMapBuilder::codeLines(cmark_node* node,
  ...
  if (fenced) {
    ...
    raw = newline == std::string::npos ? source.size() : newline + 1;
  }
  for (std::size_t begin = 0; begin < literal.size();) {
    ...
    SourceRange range =
        offset == std::string::npos
            ? SourceRange{raw, rawEnd, MappingQuality::Approximate}

...
Konverter kildetekst til en stabil, eid semantisk modell. MD4C er første adapter, ikke del av den offentlige kontrakten. Interpreter kjenner ikke fonter, FOX eller navigasjonshistorikk.
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `PreviewCoordinator::refresh` | `IInterpreter::parse / Md4cInterpreter::parse` | `src/interpreter/Md4cInterpreter.cpp` | Snapshot + profil → ParseResult | Runtimefeil avbryter modellpublisering. | Planned |
| 2 | `Md4cInterpreter::parse` | `ModelBuilder::beginDocument` | `src/interpreter/ModelBuilder.cpp` | Dokument/revisjon → builder | Ny builder per parse, ingen delt tilstand. | Planned |
| 3 | `MD4C block/span/text callbacks` | `ModelBuilder::appendEvent` | `src/interpreter/ModelBuilder.cpp` | Parserhendelse → semantikk | Bibliotektyper stopper i adapteren. | Planned |
| 4 | `ModelBuilder::appendEvent` | `SourceMapBuilder::record` | `src/interpreter/SourceMapBuilder.cpp` | Kildebevis → range/kvalitet | Ukjent posisjon merkes, gjettes ikke exact. | Planned |
| 5 | `Md4cInterpreter::parse` | `ModelBuilder::finish` | `src/interpreter/ModelBuilder.cpp` | Builder → eid SemanticDocument | Valider ranges/revisjon før retur. | Planned |

## 6. Gjenbruk og avhengigheter

Ingen andre functionality-kontrakter konsumeres; delte datatyper følger arkitekturen.

Preview er første konsument; headless inspeksjon/eksport kan senere bruke samme port. Renderer mottar bare ferdig modell. Ingen deling av MD4C callbacks med application.
...
Proposed, revisjon 0.1, 2026-09-12. P0 må bevise kildeområder: MD4C gir ikke alle ranges direkte. Alternativ interpreter/avgrenset adapterutvidelse vurderes før Ready dersom mapping ikke møter UR-008.

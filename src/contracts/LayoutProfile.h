...
struct PaperSpec {
  ...
  bool operator==(const PaperSpec& p) const {
  ...
  void validate() const {
    if (!std::isfinite(width) || !std::isfinite(height) || !std::isfinite(margin) || margin < 0 ||
        width < 72 || height < 72 || width > 2000 || height > 2000 || width - 2 * margin < 40 ||
        height - 2 * margin < 40)
...
struct FrameKey {
  ...
  bool operator==(const FrameKey& k) const {
    return token == k.token && profile == k.profile && fonts == k.fonts &&
           generation == k.generation && flowWidth == k.flowWidth;
  }
};
} // namespace xfmd
